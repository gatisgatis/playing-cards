...
import gatis.bigone.cardgames.game500.game.domain.{Game, Player, PlayerIndex}
...
object MakeBidAction {
  ...
  def apply(game: Game, bid: Int): Either[ErrorG500, Game] = for {
    ...
    _ <- checkIfPointsOver1000(game.results.gamePoints, activeIndex)
  ...
  private def checkIfPointsOver1000(
    gamePoints: Map[PlayerIndex, Int],
    activeIndex: PlayerIndex,
  ): Either[ErrorG500, Unit] = {
    val playerGamePoints = gamePoints(activeIndex)
